...
"Headers for R, FORTRAN, SAS, C, JAVA, LISP, TeX files
   autocmd bufnewfile *.R,*.Renviron,*.Rprofile so  ~/github/vim/headersandtimestamps/R_header.txt
   autocmd bufnewfile *.c,*.cpp    so  ~/github/vim/headersandtimestamps/C_header.txt
   autocmd bufnewfile *.lisp,*.lsp so  ~/github/vim/headersandtimestamps/Lisp_header.txt
   autocmd bufnewfile *.java       so  ~/github/vim/headersandtimestamps/java_header.txt
   autocmd bufnewfile *.sas        so  ~/github/vim/headersandtimestamps/SAS_header.txt
   autocmd bufnewfile *.tex        so  ~/github/vim/headersandtimestamps/TeX_header.txt
   autocmd bufnewfile *.rnw        so  ~/github/vim/headersandtimestamps/Rnoweb_header.txt
   autocmd bufnewfile *.rmd        so  ~/github/vim/headersandtimestamps/Rmarkdown_header.txt
   autocmd bufnewfile *.f,*.for    so  ~/github/vim/headersandtimestamps/Fortran_header.txt
   autocmd bufnewfile *.css        so  ~/github/vim/headersandtimestamps/web_header.txt
...
"Notice that FORTRAN positions you at the correct column (column 7)
   ...
   autocmd bufnewfile *.tex,*.rnw exe "1," . 9 . "g/For LaTeX-Box: root \=/s//For LaTeX-Box: root \= " .expand("%:r").".tex"
